...
def create_app(testing: bool = False) -> Flask:
    ...
    def api_run_once():
        ...
        _spawn_job("run-once", lambda: ShortsBotScheduler().run_single_cycle())
    ...
    def api_process_url():
        ...
        if PIPELINE_LOCK.locked():
            return _redirect_msg("Another video pipeline is currently active.", ok=False)
        started = _spawn_job("process-url", lambda: process_single_url(url, account=acc))
        if not started:
    ...
    def api_account_settings_save():
        ...
        for field in ["max_daily_uploads", "shorts_per_video", "min_minutes_between_uploads",
...
def _render_page(msg: str = "", msg_type: str = "ok", loaded_account: Optional[str] = None) -> str:
    ...
    acc_settings = {
        ...
        "aspect": _aset("aspect", _get_env_setting("SHORT_ASPECT", SHORT_ASPECT)),
...
<body>
    ...
    <div class="card">
      ...
      <form action="/api/accounts/save" method="POST">
        ...
        <div class="row" style="margin-top:10px;">
          ...
          <input type="hidden" name="acc_processmode_0" value="render">
          <span style="font-size:12px;color:var(--muted);">clip + render mode</span>
  ...
  <div class="card">
      <h2>🔗 Process one source video</h2>
    <form action="/api/process-url" method="POST">
      <div class="row">
        <input type="text" name="url" placeholder="Paste a YouTube video URL..." required>
        ...
        <button class="cyan" type="submit">Create Clip</button>
      </div>
    </form>
  </div>
...
def run_webui(host: str = WEBUI_HOST, port: int = WEBUI_PORT) -> None:
    ...
    print(f"\n  Shorts Bot Control Panel: http://{host}:{port}\n")
    app = create_app()
