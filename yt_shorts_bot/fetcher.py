...
class YouTubeFetcher:
    ...
    def _cookies_opts() -> dict:
        ...
        # If we enable proxy via .env, inject it here
        import os
    ...
    def _slice_av_pair(self, video_url: str, clip_start: float, clip_end: float, output_path: Path) -> Path:
        ...
        ydl_opts = {
            "format": "bestvideo[height<=2160][vcodec^=avc1][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=2160][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=2160]+bestaudio/best",
            "quiet": True,
            "no_warnings": True,
            **self._cookies_opts(),
            **self._ffmpeg_opt(),
